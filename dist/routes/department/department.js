"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
exports.__esModule = true;
var express_1 = require("express");
var router = (0, express_1.Router)();
var lib_1 = __importDefault(require("../../lib/lib"));
router.get("/", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var departments;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, lib_1["default"].department.findMany({
                    include: {
                        Employee: true
                    }
                })];
            case 1:
                departments = _a.sent();
                return [2 /*return*/, res.status(200).json({ departments: departments })];
        }
    });
}); });
router.post("/add", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var name, department, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                name = req.body.name;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, lib_1["default"].department.create({
                        data: {
                            name: name
                        },
                        select: {
                            id: true,
                            name: true,
                            Employee: true
                        }
                    })];
            case 2:
                department = _a.sent();
                return [2 /*return*/, res.status(200).send(department)];
            case 3:
                error_1 = _a.sent();
                if (error_1.code === "P2002") {
                    return [2 /*return*/, res.status(400).send("Department name already exists")];
                }
                console.log(error_1);
                res.status(400).json({ error: error_1 });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
router
    .route("/:id")
    .get(function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, department, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, lib_1["default"].department.findUniqueOrThrow({
                        where: {
                            id: parseInt(id)
                        }
                    })];
            case 2:
                department = _a.sent();
                return [2 /*return*/, res.status(200).json({ department: department })];
            case 3:
                error_2 = _a.sent();
                console.log(error_2);
                return [2 /*return*/, res.status(400).json({ error: error_2 })];
            case 4: return [2 /*return*/];
        }
    });
}); })["delete"](function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, department, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, lib_1["default"].department["delete"]({
                        where: {
                            id: parseInt(id)
                        }
                    })];
            case 2:
                department = _a.sent();
                return [2 /*return*/, res.status(200).json({ department: department })];
            case 3:
                error_3 = _a.sent();
                console.log(error_3);
                return [2 /*return*/, res.status(400).json({ error: error_3 })];
            case 4: return [2 /*return*/];
        }
    });
}); })
    .patch(function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, name, department, error_4;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                name = req.body.name;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, lib_1["default"].department.update({
                        where: {
                            id: parseInt(id)
                        },
                        data: {
                            name: name
                        }
                    })];
            case 2:
                department = _a.sent();
                return [2 /*return*/, res.status(200).json({ department: department })];
            case 3:
                error_4 = _a.sent();
                console.log(error_4);
                return [2 /*return*/, res.status(400).json({ error: error_4 })];
            case 4: return [2 /*return*/];
        }
    });
}); });
exports["default"] = router;
//# sourceMappingURL=department.js.map